...
`export` collects the transitive `deps` closure of every target and stores the bundle as a raw CAS object, so it needs the define scope like `POST /v1/cas`; the CLI claims `--out` before sending the request (an existing file refuses the export) and renames the downloaded bytes into place, so the file is either absent or complete. A target given as a hash must be the current value of a name. `import` uploads the file to `POST /v1/cas`, then runs the `import` verb with the returned reference: every block is hashed against its CID, every listed object must be reached from a definition record, every root name and `--into` pass the one name rule (no leading `#`, not hash-shaped, no empty segment), a Rust record's resolver key must equal the key this node derives for it, every definition is rebuilt through the same admission path as `add` in dependency order, and the whole bundle is refused when a rebuilt hash differs from the recorded one (the error names both hashes and both toolchain hashes) or when a bound name already points at a different hash. `--into friend` binds `friend/<name>` instead. Nothing reaches the live store unless every definition rebuilt; see [bundles.md](bundles.md) for the byte-level format and the object list.
...
For a Rust definition `view` also returns `result.formatted_source`, the stored source run through the pinned toolchain's `rustfmt` (`--edition 2024`, source on stdin, ten-second limit, the same cleared environment as the compiler). `result.source` stays the submitted bytes; the formatted text is a second field, recorded once per source revision in the CAS (kind `source-formatted`) and reused by later views. When rustfmt refuses the source or is unavailable, `formatted_source` is `null` and `format_error` holds rustfmt's message; the unformatted text is never returned under the formatted name. For a source bundle (`{"files": ...}`) the formatted file is `src/lib.rs`, the file `source` shows. JavaScript and TypeScript definitions have neither field.
