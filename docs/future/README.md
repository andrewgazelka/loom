...
| [dependency-rlib-reuse.md](dependency-rlib-reuse.md) | a caller with a new definition-dependency set replays instead of a 27 s cold Cargo resolve | rlib capture landed (`direct/definition_rlibs.rs`); the graph key still includes the dependency set | `scripts/bench/add-latency.sh` dependent-add mode prints under 2 s; cross-graph link test passes |
...
| [ingress-batching.md](ingress-batching.md) | cross-node sends cost two ships each today (measured 200-500 ms per message) | cluster lane landed | 100-message FIFO test drains in under 10 s on hydra (debug) |
...
| [cargo-unit-hash-rustc.md](cargo-unit-hash-rustc.md) | one Rust build path in the repo instead of two; per-unit rebuilds for the driver | the two cargoUnit graph stages are told apart, or Nix on macOS stops denying `/private/etc/ssl/openssl.cnf` | `nix/hash-rustc.nix` renders through cargoUnit and `hash-rustc -vV` still matches the guest `rustc -vV` |
...
| [nix-host-checks.md](nix-host-checks.md) | `nix build .` builds the host binaries but checks nothing; cargoUnit can render clippy and the tests per unit | CI exists | `nix flake check` fails on a planted warning and a planted failing test, and rebuilds nothing on an unchanged tree |
