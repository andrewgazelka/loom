...
pub fn handle(op: loom::Op, _continuation: loom::Continuation) -> loom::Reply {
    if op.label == "sleep" {
...
loom::handle_with("<64-character definition hash>", || {
    // Operations here run under the stored handler.
})
```
...
The checker resolves this syntax into an ordinary `loom::handle` call to the
...
is rejected explicitly. Effect-row analysis still needs to account for operations
performed by the handler itself in its outer context; storing a handler does not
grant additional host abilities or establish a proof of compiler soundness.
