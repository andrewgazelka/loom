...
2. **Calling an effect performs it; only the outermost handler is the host.** `loom.perform` dispatches through any installed guest handler stack before reaching the host dispatcher; recording and replay observe only that root. `crates/loom-rt/src/root_handler.rs`, ABI in `docs/shared-core-abi.md`. An isolated call (`loom::isolated::call`, import `loom.call`) is not a perform: it is the isolation boundary between definitions, the host reads only its fixed header and never decodes the opaque DAG-CBOR payload (two codec passes per direction, both in guests), and one function decides target, policy, depth and arity: `crates/loom-rt/src/isolated.rs`, wire in `crates/loom-proto/src/isolated.rs`. Code that merely calls code links statically by hash instead.
...
4. **Effect rows are inferred, never declared, and enforced at the root.** The rustc driver (`tools/hash-rustc/src/effects.rs`) computes each entry's reachable host effects through the resolved call graph (trait and generic calls included) and removes labels handled by total handlers; `loom-check` (`crates/loom-check/src/driver_effects.rs`) consumes that row and rejects a `perform` whose label is not a literal or a const at its call site; the host refuses any effect outside the row at run time. Guest code carries no export attributes and no declarations; built-in derives, std macros, and local `macro_rules!` expand inside rustc before both passes run (`docs/content-addressed-code.md`, "Macros in guest source"), while procedural macros are refused by `crates/loom-check/src/rust_effects/macros.rs`.
...
| `loom-guest-rs` | execution | `crates/loom-guest-rs/src/lib.rs` — synchronous Rust guest API, `scope`/`spawn`, `handle`/`handle_any`; no guest macro crate |
