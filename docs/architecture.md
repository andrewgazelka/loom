...
4. **Effect rows are inferred, never declared, and enforced at the root.** The rustc driver (`tools/hash-rustc/src/effects.rs`) computes each entry's reachable host effects through the resolved call graph (trait and generic calls included) and removes labels handled by total handlers; `loom-check` (`crates/loom-check/src/driver_effects.rs`) consumes that row and rejects a `perform` whose label is not a literal or a const at its call site; the host refuses any effect outside the row at run time. Guest code carries no macros and no declarations.
...
| `loom-guest-rs` | execution | `crates/loom-guest-rs/src/lib.rs` — synchronous Rust guest API, `scope`/`spawn`, `handle`/`handle_any`; no macros |
