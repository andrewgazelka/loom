...
4. **Inferred residual effects are enforced at the root.** The rustc driver follows resolved calls and removes labels handled by total handlers. The checker rejects runtime-selected effect labels at their call sites; the host enforces the inferred row. `tools/hash-rustc/src/effects.rs`, `crates/loom-check/src/driver_effects.rs`.
