...
15. **One guest language.** Rust is the only supported guest language going forward; TypeScript and the WIT component path are being removed. `loom-guest-ts/`, `loom-wit/handler.wit`, and TS handling in `crates/loom-check` and `crates/loom-build` still exist in the tree today (round 3, in progress).
