...
4. **Effect rows are inferred, never declared, and enforced at the root.** The rustc driver (`tools/hash-rustc`) computes each entry's reachable host effects through the resolved call graph (trait and generic calls included); `loom-check` consumes that row; a `perform` whose label is not a literal or a const is a compile error at that line; the host refuses any effect outside the row at run time. Guest code carries no macros and no declarations (round 5, lane r5-effects).
