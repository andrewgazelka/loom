...
const sources = import.meta.glob<string>(['../../../../*.md', '../../../../../README.md'], { query: '?raw', import: 'default', eager: true });
...
async function buildDocs(): Promise<DocPage[]> {
  ...
  const docs = Object.keys(sources).map((path) => parseDoc(path.endsWith('/README.md') ? 'README.md' : `docs/${path.split('/').at(-1)}`, sources[path]));
