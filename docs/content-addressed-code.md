...
```json
{
  ...
  "exports": { "entry": "same hash as items.entry.hash", "shapes::largest": "64 lowercase hex digits" },
  "effects": {
...
Entries are ordinary public functions at the crate root, including `main`. Multiple root `pub fn` items are allowed. Private functions, nested public functions, and types are not entries. Guest source may use built-in derives, the toolchain's own declarative macros, and its own `macro_rules!` ([Macros in guest source](#macros-in-guest-source)); it uses no procedural or attribute macros, and the SDK has no guest macro crate. Entry discovery reads rustc's resolved item kind, parent, and visibility, after expansion. Entries drive the Wasm ABI (`loom-build/src/direct/entry_abi.rs`); they are not the identity surface.

Exports are every supported definition that rustc's effective-visibility analysis marks reachable from outside the crate (`tcx.effective_visibilities(()).is_reachable`): `pub` items inside `pub` modules, items reached through `pub use`, impl blocks and impl members of reachable types (inherent members by their own visibility, trait-impl members through the trait), trait members of reachable traits, and types that a public signature leaks without naming. A `pub fn` inside a private module is not an export; a private helper never is. Every entry is an export, and the driver asserts it. Exports carry the same hashes as `items`; the field exists so a consumer can take the public API surface without recomputing visibility.

## Identity rules

Each definition becomes an ordered, framed HIR stream, prefixed with `loom-hir-v4`. Node kinds, operators, literals, signatures, explicit generic arguments, bounds, pattern structure, and statement order are retained. Span offsets, hygiene numbers, HIR allocation IDs, local variable spellings, and ordinary function names are absent. Local uses encode the reverse position of their resolved binder in the lexical binding stack, so shadowed variables remain distinct. Generic parameters use declaration positions. Labels encode their resolved control-flow target's position. Generic bodies are hashed once, without monomorphization.
...
An ordinary local reference contributes the referent's 32-byte hash. Constructors, variants, and fields, local or external, contribute their structural position and the enclosing type's hash; they are never referents of their own. Hashing proceeds from dependency sinks toward callers. For a strongly connected component, content-based partition refinement assigns member classes: begin with one class, group lexicographically by the previous class index and encoded stream, and repeat until no class splits. Internal references use the previous round's class indices; outgoing references use resolved hashes. Equivalent recursive definitions share a class. Concatenate one length-framed stream per final class and BLAKE3-hash that cycle. Each member gets `blake3(cycle_hash || u64_le(class_index))`. No diagnostic path determines ordering or enters the hash. This also handles the cycles introduced by ADT → impl → method → Self dependencies without requiring a cryptographic fixed point.

An external reference has two rules, selected by the referent's crate. When the crate is a Loom definition dependency, the reference is the referent's stored item hash: the 32-byte value recorded under the referent's own definition path in that dependency's `items.json`. The caller's identity is then a function of the callee's content, and an unrelated helper edit, a formatting change, or a different `-C metadata` in the dependency leaves the caller's hash unchanged (`tests/dependencies.rs`). When the crate is anything else (std, the SDK, crates.io), the reference is the whole-crate rule `blake3(u64_le(crate_name_utf8.len) || crate_name_utf8 || u128_le(crate_hash) || DefPathHash_bytes)`, where the crate hash is rustc's strict version hash from `tcx.crate_hash`, not its stable crate ID. The fixed-metadata dependency regression changes such a dependency's body while preserving its name and metadata argument, and verifies that the referring item changes.

The build names the dependency documents through `LOOM_DEP_ITEMS=<directory>`: `<directory>/<crate_name>.json` is the `items.json` that crate's own definition build recorded, and a crate is a Loom definition dependency exactly when such a file exists. `crate_name` is rustc's name for the crate (`tcx.crate_name`, `loom_definition_<hash16>` for a materialized definition), never the `--extern` alias. The lookup key is the referent's verbose disambiguated definition path without any crate prefix, `tcx.def_path(id).to_string_no_crate_verbose()` with the leading `::` removed; that is the spelling the dependency's own build wrote into its `items` map, since local items carry no crate prefix. `refs` still spells the same referent with the crate prefix. A staged crate whose document lacks the path is a fatal error naming the crate, the path, and the document file; the whole-crate rule is not a fallback. An entry under the directory that is not `<crate_name>.json`, an unreadable document, or a malformed hash is also fatal. Unset, or an empty directory, means no crate is content-linked. Instantiated (mono) identities and object-cache keys keep the whole-crate rule for every external crate: an object is compiled against one specific dependency build.
...
The first token is atom `loom-hir-v4`: `00 0b 00 00 00 00 00 00 00` followed by its eleven UTF-8 bytes. Atoms are generated in the order specified by `tools/hash-rustc/src/encode.rs` and `src/encode/*.rs`. These source files are the normative v4 node schema, together with `src/graph.rs` (reference expansion and the two external rules) and `src/graph/canonical.rs`. Structural traversal uses this pinned rustc's `rustc_hir::intravisit` walkers except where those encoders explicitly replace traversal. Another HIR producer must implement that schema, not use pretty-printed Rust or rustc's incremental hash.
...
## Macros in guest source

Until 2026-09-21 `loom-check` refused every macro invocation, every `macro_rules!` definition, and every attribute outside a short inert list, with diagnostic code `LOOM_MACRO`. The ban arrived in commit `2b594d1` ("Infer effect rows from resolved calls and export root public functions"), the change that deleted the SDK's procedural-macro crate `crates/loom-guest-macros` (which had carried `#[loom::def(effects = [...])]` export and effect declarations) and replaced it with "every crate-root `pub fn` is an entry, effects are inferred". Nothing in the new design needed a macro, so `crates/loom-check/src/rust_effects/macros.rs` refused all of them ("executable expansion is unavailable"). Read against the code, none of the three plausible reasons for keeping the blanket ban holds:

- **Identity is computed after expansion.** The driver reads HIR once name resolution, macro expansion, and type checking are complete (`tools/hash-rustc/src/main.rs`, `after_analysis`). `graph.rs::collect` enumerates `tcx.iter_local_def_id()`, which includes expansion-generated impls and methods, and `graph.rs::implementations` attaches derive-generated impl blocks to their ADT like any hand-written impl. Expanded literals are real HIR content (above). The coverage census on `crates/loom-guest-rs`, a crate that itself uses `vec!`, `serde_json::json!`, `format!`, and serde derives, refused 45 of 209 HIR items, every one for `type-relative path outside body` (since closed) and none for anything produced by expansion.
- **The authoritative effect pass runs after expansion.** The source pass in `crates/loom-check/src/rust_file.rs` marks every export `unknown: true`; rows come only from `tools/hash-rustc/src/effects.rs`, whose `hir::scan` walks the expanded body and whose `mir::scan` walks MIR (`crates/loom-check/src/driver_effects.rs`: "the single authority for residual host rows"; `docs/architecture.md`, principle 4). The `syn` passes (`LOOM_UNSAFE`, `LOOM_IO`, `LOOM_MACRO`) are source admission. They see macro bodies and macro inputs as token trees and scan them as such (`rust_effects/admission.rs` for `unsafe`, `rust_file.rs` for ambient input and `std::{fs,net,time,env,process}`, `rust_effects/macros.rs` for nested `name!(...)` and `#[...]` shapes), so a `macro_rules!` body cannot hide from them what its expansion would show the compiler.
- **Compile-time execution is gated in the build layer, not in this check.** A procedural macro runs on the build host. `crates/loom-build/src/direct/admission.rs::graph_shareable` requires every `proc-macro` or `custom-build` package, and its transitive dependencies, to be `trusted_sources::approved` (a repository SDK path, or a crates.io archive whose checksum matches the SDK's `Cargo.lock`), and `compile.rs` refuses the build otherwise; `rustc/capture.sh` refuses untrusted `--crate-type proc-macro` and `build_script_build` compilations outright; `crates/loom-check/src/safety.rs` refuses untrusted packages that declare themselves procedural macros. Built-in derives are implemented inside rustc and `macro_rules!` is substitution; neither executes user code.

The rule is therefore, in `crates/loom-check/src/rust_effects/macros.rs`, with one table per class driving both admission and the diagnostic text:

| Class | Rule | Reason |
| --- | --- | --- |
| `#[derive(...)]` | Every path must be a built-in derive: `Clone`, `Copy`, `Debug`, `Default`, `Eq`, `Hash`, `Ord`, `PartialEq`, `PartialOrd`, bare or as `core::<module>::Name` / `std::<module>::Name`. `#[default]` on a variant is admitted with it. | Implemented in rustc; the generated impls are HIR, hashed as part of the ADT's identity, and effect-inferred. |
| Toolchain macros | Each macro bare or under the roots that export it, and no others: `assert`, `assert_eq`, `assert_ne`, `concat`, `debug_assert`, `debug_assert_eq`, `debug_assert_ne`, `format_args`, `matches`, `panic`, `stringify`, `todo`, `unimplemented`, `unreachable`, `write`, `writeln` under `core::` / `std::`; `format` and `vec` under `alloc::` / `std::`; `json` under `serde_json::` / `loom::serde_json::`. `alloc::assert!` and `core::vec!` are refused. | Declarative or compiler-built-in; no user code at compile time; the expansion is HIR. `serde_json` is a checksum-pinned SDK dependency and `json!` is `macro_rules!`. |
| Local `macro_rules!` | Definitions are admitted as items (at the crate root, in a module, or in a block), never inside another macro's body. Every arm must read as `(matcher) => {transcriber}`; every fragment in a matcher must use one of `block`, `expr`, `expr_2021`, `ident`, `item`, `lifetime`, `literal`, `meta`, `pat`, `pat_param`, `path`, `stmt`, `ty`, `vis` (`tt` is refused). Invocations are admitted by bare name when the same file defines the macro. | The expansion is HIR and is hashed and inferred like written code; the transcriber is scanned as tokens by every source pass. Every admitted fragment kind binds syntax the call site spells in full, so the call-site scan sees what the fragment carries; a `tt` fragment binds a bare `#`, `!` or `[...]` that a transcriber could pair with other tokens into an attribute or invocation neither side shows. |
| Attributes | `allow`, `cfg(test)` exactly, `cold`, `default`, `deny`, `deprecated`, `doc`, `expect`, `forbid`, `inline`, `must_use`, `non_exhaustive`, `repr`, `test`, `track_caller`, `warn`. `#[default]` is admitted wherever it is written; rustc refuses it anywhere but on a unit variant of an enum deriving `Default`. | Inert, or (`cfg(test)`, `test`) removed by rustc outside `--test`: the guest build never runs tests, so those items have no identity and no effects while the source passes still scan them. Any other `cfg` predicate would give the source a shape the one guest build lacks. |
| Fragments in macro-name or attribute position | Inside a macro body: `$m!(...)`, `$m::name!(...)`, `$crate::name!(...)`, `$(...)*!(...)`, `#[$a]`, `#![$a]`, `#[derive($d)]`, and any `#` not immediately followed by `[...]` are refused, naming the fragment. `LOOM_IO` refuses `$m!` and `$(...)*!` independently. | The call site spells the macro name without `!` and the attribute without `#`, so neither side presents a shape the tables can match; `call!(include_str)` with a body of `$m!("/etc/passwd")` expands to `include_str!`. |
| Import aliases | `use path as Name` (as syntax, or as the tokens `as Name` inside a macro body) is refused when `Name` is any name in the derive, macro or attribute tables, or `derive` / `macro_rules`. `use std::fmt::Write as FmtWrite` and `as _` are admitted. | The tables match spelling: `use loom::serde::Serialize as Clone; #[derive(Clone)]` would run `serde_derive`, and `use std::println as format;` would reach stdio. |
| Everything else | Refused, naming the macro, derive, attribute, fragment or alias and printing the table. | Fail closed. |

The build layer closes the remaining route. A `macro_rules!` transcriber may still produce a crate-root `pub fn` (`macro_rules! entry { ($v:vis $n:ident) => { $v fn $n() {} } }`), which rustc counts as an entry the source pass never listed as an export. `crates/loom-build/src/identity.rs::ingest` requires the compiler's entry set and the checker's export set to be equal in both directions and rejects the build naming the odd item.

Deliberate exclusions: `print!`, `println!`, `eprint!`, `eprintln!`, `dbg!` (the `wasm32-unknown-unknown` guest has no stdio and std silently discards the bytes, a success-lookalike); `file!`, `line!`, `column!`, `module_path!` (they bind identity to source layout, which formatting must not move); `include!`, `include_str!`, `include_bytes!`, `env!`, `option_env!` (compile-time ambient input, also refused as `LOOM_IO`); `cfg!`, `cfg_attr`, `thread_local!`, `compile_error!`, `macro_export`, `macro_use` (one build configuration, one file scope); the `tt` fragment specifier and `macro_rules!` inside a macro body (above). A local macro reached through a path (`self::helper!`, `crate::helper!`) is refused; textual scoping needs only the bare name. `loom::handle_with` lowering (`handler_references.rs`) does not see macro tokens; an expansion that produces `loom::handle_with(...)` fails to compile because the SDK exports only `handle_pinned`, so that path fails closed too.

Procedural macros from crates, `serde_derive` included, remain refused at the source level even though the build layer already admits and executes `serde_derive` as a trusted SDK dependency (the guest `Cargo.toml` template in `crates/loom-build/src/intake.rs` and `materialize.rs` enables `serde` with the `derive` feature). Admitting a trusted, vendored procedural derive would take exactly three things: a row in the derive table naming the derive path and the crate that implements it (`serde::Serialize` and `serde::Deserialize`, implemented by `serde_derive`); the execution gate that already exists, `crates/loom-build/src/direct/admission.rs::graph_shareable` with `trusted_sources.rs::approved`, which lets the proc-macro run only inside the build workspace (`rustc/sandbox.sh` under bubblewrap when isolated) and only when its source is the checksum-pinned archive reachable from the SDK's `Cargo.lock`; and nothing for identity, because the derive's output is local HIR whose references to `serde` items carry `serde`'s crate hash, so a `serde_derive` change that alters the expansion changes the definition hash and one that does not, does not. What it must not take is a name-based grant: `trusted_sources.rs` states "A package name alone cannot grant trust".

Changing `rust_effects/macros.rs` changes `safety_policy_bytes()` (`crates/loom-check/src/safety.rs`), which invalidates cached build artifacts admitted under the previous policy. The `hash-rustc` fixtures `builtin_derives_are_expanded_impls_inside_adt_identity`, `builtin_derives_on_enums_expand_to_supported_hir`, `format_and_vec_expansions_hash_their_literals_not_their_spellings`, and `matches_assert_and_write_expansions_hash` in `tools/hash-rustc/tests/identity.rs` pin the encoder's handling of expanded HIR: a derive changes the ADT hash and its users' hashes, a change inside a `format!` string or a `vec!` literal moves the entry hash, and renaming a local captured by a format string does not.

## Integration with loom-build

`loom-build::identity` validates the compiler document and rehashes item and cycle
preimages before importing them into the CAS. The definition identity is the
BLAKE3 root of `loom_proto::export_identity_preimage`: the domain
`loom:definition:exports:v1`, the export count, then every `exports` path/hash
pair in path order, each length-framed. A document without `exports` is
rejected by field name; nothing is recomputed from `entry`. Every entry must
appear among the exports and every export must agree with `items`. Stored
...
as `<identity directory>/dependency-items/loom_definition_<hash16>.json` and
sets `LOOM_DEP_ITEMS` to that directory beside `LOOM_ITEM_HASHES` and
`LOOM_ITEM_PREIMAGES`. A dependency with no stored build identity or document
fails the build by alias and hash. The staged documents are inputs and are not
published with the side outputs.
...
The definition identity is the Merkle root over `exports`, every definition reachable through `pub` from the crate root, so a nested `pub fn` change moves it even when no entry reaches that function.
A reference into another Loom definition carries that item's stored hash; a reference into std or the SDK carries the whole-crate digest.
...
The lowered-code refinement is necessary: the HIR identity above deliberately omits selected trait implementations, implicit adjustments, and optimization hints. Reusing objects under that identity alone could execute stale code. The HIR stream is v4; the item document adds `exports` beside `entry`, and mono identities keep the whole-crate rule for external referents. Compiler-private debug representations used for lowered scalar operations and settings are covered by the exact compiler version in the key; they are not a cross-nightly interchange format.
...
The SDK smoke test exposed eager HIR hashing of unsupported `Invocation::arguments` (today `loom::isolated::Invocation::encode_args`) before CGU eligibility was known. The cache now checks lowered-code eligibility first and skips HIR hashing and store creation when no CGUs are eligible. A regression covers that exact `Self::Args` signature shape. The earlier 48-module micro-fixture remains 456.571 ms cached versus 387.252 ms uncached with 47 hits; the new real measurements establish the current coverage limit rather than a general performance verdict. The resulting default remains disabled.
...
- An ordinary function instance hashes as `blake3(generic_definition_hash_bytes || canonical_generic_args)`. The definition digest is raw 32-byte BLAKE3. External definitions use the whole-crate rule above (framed crate name, strict crate hash, DefPathHash) regardless of `LOOM_DEP_ITEMS`; content-linked references are a HIR-identity rule only. Constructors, closures, coroutines, and other nested definitions without an independent HIR owner use a tagged nested-definition digest containing their kind, lexical disambiguator, enclosing definition's content hash, and a variant index where applicable. Their enclosing HIR contains the nested body.
