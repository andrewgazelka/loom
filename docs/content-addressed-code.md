...
| Toolchain macros | `assert`, `assert_eq`, `assert_ne`, `concat`, `debug_assert`, `debug_assert_eq`, `debug_assert_ne`, `format`, `format_args`, `matches`, `panic`, `stringify`, `todo`, `unimplemented`, `unreachable`, `vec`, `write`, `writeln`, bare or under `core::` / `alloc::` / `std::`; `json` bare or under `serde_json::` / `loom::serde_json::`. | Declarative or compiler-built-in; no user code at compile time; the expansion is HIR. `serde_json` is a checksum-pinned SDK dependency and `json!` is `macro_rules!`. |
| Local `macro_rules!` | Definitions are admitted anywhere. Invocations are admitted by bare name when the same file defines the macro. | The expansion is HIR and is hashed and inferred like written code; the body is scanned as tokens by every source pass. |
| Attributes | `allow`, `cfg(test)` exactly, `cold`, `default`, `deny`, `deprecated`, `doc`, `expect`, `forbid`, `inline`, `must_use`, `non_exhaustive`, `repr`, `test`, `track_caller`, `warn`. | Inert, or (`cfg(test)`, `test`) removed by rustc outside `--test`: the guest build never runs tests, so those items have no identity and no effects while the source passes still scan them. Any other `cfg` predicate would give the source a shape the one guest build lacks. |
| Everything else | Refused, naming the macro, derive or attribute and printing the table. | Fail closed. |

Deliberate exclusions: `print!`, `println!`, `eprint!`, `eprintln!`, `dbg!` (the `wasm32-unknown-unknown` guest has no stdio and std silently discards the bytes, a success-lookalike); `file!`, `line!`, `column!`, `module_path!` (they bind identity to source layout, which formatting must not move); `include!`, `include_str!`, `include_bytes!`, `env!`, `option_env!` (compile-time ambient input, also refused as `LOOM_IO`); `cfg!`, `cfg_attr`, `thread_local!`, `compile_error!`, `macro_export`, `macro_use` (one build configuration, one file scope). A local macro reached through a path (`self::helper!`, `crate::helper!`) is refused; textual scoping needs only the bare name. `loom::handle_with` lowering (`handler_references.rs`) does not see macro tokens; an expansion that produces `loom::handle_with(...)` fails to compile because the SDK exports only `handle_pinned`, so that path fails closed too.
