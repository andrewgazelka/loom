...
`step_v1` and the fixed `step` return the same effects and state, so the fix changes
behaviour only where the bug lives (the "fix agreement" check from Boris Cherny's
chart). `Codec.lean`: the phase storage codec round-trips.
