...
<section class="filesystem-changes">
  ...
  {#each changes as item, index}
    ...
    <details bind:open={expanded[String(index)]}><summary>{String(change.path || "File")} <span>{String(change.machine || "")}</span></summary>
      {#if expanded[String(index)]}{#if change.captured === true && (typeof change.before === "string" || change.before === null) && (typeof change.after === "string" || change.after === null)}<TextDiff {client} {inspect} before={change.before} after={change.after} />{:else}<p>Before/after content was not captured for this path.</p>{/if}{/if}
    </details>
  {/each}
  {#each unavailable as item}<p>{String(record(item).path)}: {String(record(item).reason || "Capture unavailable")}</p>{/each}
  {#if capture.unavailable_reason}<p>{String(capture.unavailable_reason)}</p>{:else if !changes.length && !unavailable.length}<p>{capture.scope === "explicit_paths" ? "No captured path changed." : "Before/after content was not captured. No file diff is available."}</p>{/if}
</section>
<style>
