...
<svelte:window on:pointerdown={(event) => { if (!root?.contains(event.target as Node)) open = false; }} />
<div class="select" bind:this={root}>
  ...
  {#if open}<div class="options" role="listbox" {id} aria-label={label}>
...
<style>
  ...
  .options { position:absolute; z-index:30; top:calc(100% + 6px); left:0; min-width:160px; max-width:350px; max-height:260px; overflow:auto; padding:4px; border:1px solid var(--line); border-radius:7px; background:var(--card); box-shadow:0 8px 24px #0002; }
