...
COPY . .
RUN cargo build --locked --release -p loomd -p loom-cli
...
WORKDIR /opt/loom
COPY --from=build /opt/loom/target/release/loomd /usr/local/bin/loomd
COPY --from=build /opt/loom/target/release/loom /usr/local/bin/loom
