...
try {
  ...
  async function invoke(lang:string):Promise<void>{
    const response=await client!.callTool('loom_run',{target:hashes[lang],args:[machine.id,'.']});
  ...
  for(const lang of ['rust'])await gate(`${lang} model-written recursive definition executes correctly`,async()=>{
    const definitions=calls.filter(call=>call.server==='loom'&&call.tool==='loom_add'&&successfulLoomResult(call)&&object(call.arguments).name===`codex-recursive-${lang}`);
    ...
    const viewed=await client!.callTool('loom_view',{target:traced.hash});
    ...
    assert(calls.some(call=>call.server==='loom'&&call.tool==='loom_run'&&successfulLoomResult(call)&&object(call.arguments).target===definition.hash),`trace lacks actual${lang}execution`);
    await invoke(lang);
  });
