...
test('negotiates protocol, decodes chunked SSE, preserves diagnostics, closes session', async () => {
  ...
  try { await client.connect(); const reply=await client.callTool('add',{}); expect(reply.ok).toBe(false); expect(reply.diagnostics).toHaveLength(1); await client.close(); expect(closed).toBe(true); }
...
test('tool errors remain transport errors', async () => {
  ...
  try { const client=new LoomMcpClient({endpoint:server.url.origin,token:'test'}); await expect(client.callTool('add',{})).rejects.toBeInstanceOf(McpTransportError); }
  finally { server.stop(true); }
});
