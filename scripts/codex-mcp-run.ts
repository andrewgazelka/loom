...
Rust single-file pub fn main(machine:String,path:String)->loom::Value. Every root pub fn is an entry; keep traversal helpers private. Guest code has no export attributes or effect declarations; the driver infers effects. loom::fs::list(&machine, &path) -> Result<Vec<loom::DirEntry>,String>. Construct loom::Value objects with loom::serde_json::json!, maps, or Value constructors; std macros such as vec!, format!, matches! and assert! are available, procedural macros (serde derives, tokio::main) and println!/dbg! are not. Use loom::scope with s.spawn(|| loom::fs::list(&machine, &path)) and child.join() for concurrent directory listings. Implement traversal in guest. No exec/std::fs/fs.snapshot. Correctness and fast full-tree scans matter.`);
