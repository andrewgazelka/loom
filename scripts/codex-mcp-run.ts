...
args.push(`Use only Loom MCP tools, no shell/web/other tools. Create machine with loom_command machine.create args {root:${JSON.stringify(fixture)}}. Write a recursive largest regular-file definition named codex-recursive-rust with lang rust. It accepts (machine:string,path:string) and returns {path:string,size:number}, path relative to supplied root without ./ prefix. Skip symlinks and special files. Traverse every nested directory; ties choose lexicographically smallest path. Call it via loom_command call {hash,args:[machineId,"."]} and verify the result. Retry compiler diagnostics until it works. Final JSON {rust_hash}.
Rust single-file #[loom::def(effects=["fs.list"])] pub fn main(machine:String,path:String)->loom::Value. loom::fs::list(&machine, &path) -> Result<Vec<loom::DirEntry>,String>. loom::serde_json::json! available. Use loom::scope with s.spawn(|| loom::fs::list(&machine, &path)) and child.join() for concurrent directory listings. Implement traversal in guest. No exec/std::fs/fs.snapshot. Correctness and fast full-tree scans matter.`);
