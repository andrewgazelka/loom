...
for(const tool of ['loom_add','loom_run','loom_view','loom_command'])args.push('-c',`mcp_servers.loom.tools.${tool}.approval_mode="approve"`);
args.push(`Use only Loom MCP tools, no shell/web/other tools. Create machine with loom_command machine.create args {root:${JSON.stringify(fixture)}}. Write a recursive largest regular-file definition named codex-recursive-rust using loom_add {source,name}. It accepts (machine:string,path:string) and returns {path:string,size:number}, path relative to supplied root without ./ prefix. Skip symlinks and special files. Traverse every nested directory; ties choose lexicographically smallest path. Call it via loom_run {target:hash,args:[machineId,"."]} and verify result.output. Retry compiler diagnostics until it works. Final JSON {rust_hash}.
