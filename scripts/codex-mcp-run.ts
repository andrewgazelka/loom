...
TS imports from "loom": fs.list({machine,path}) returns Value entries {name,size,is_dir,is_file,is_symlink}. Export main, strict types, no any/async/nativeIO. Batch frontier using all(fs.list.desc(...)). Rust single-file #[loom::def] pub fn main(machine:String,path:String)->loom::Value. loom::abilities::fs::list(loom::Value::String(machine.clone()), &path) -> Result<loom::Value,String>. serde_json::json! available. Batch using loom::Desc::<loom::Value>::new("fs.list",serde_json::json!({"machine":machine,"path":path})) and loom::all(descs). Implement traversal in guest. No exec/std::fs/fs.snapshot. Correctness and fast full-tree scans matter.`);
