...
try {
  ...
  await gate(names[0]!,async()=>{
    positive=await define('nested',`fn nested(depth:u32)->u32 {if depth==0 {1} else {loom::scope(|s|s.fork(||nested(depth-1)).expect("fork").join().expect("join")+1).expect("scope")}} #[loom::def] pub fn main()->u32 {nested(16)}`);
  ...
  await gate(names[1]!,async()=>{
    const hash=await define('jobs',`#[loom::def] pub fn main()->u32 {loom::scope(|s| {let mut count=0; for _ in 0..513 {match s.fork(||1_u32) {Ok(job)=>{count+=job.join().expect("join");},Err(_)=>return count}} count}).expect("scope")}`);
  ...
  await gate(names[2]!,async()=>{
    const hash=await define('stack',`#[inline(never)] fn small()->u8 {let bytes=std::hint::black_box([7_u8;1024]);std::hint::black_box(&bytes)[0]} #[inline(never)] fn large()->u8 {let bytes=std::hint::black_box([7_u8;524288]);std::hint::black_box(&bytes)[0]} #[loom::def] pub fn main(big:bool)->u8 {loom::scope(|s|s.fork(||if big {large()} else {small()}).expect("fork").join().expect("join")).expect("scope")}`);
  ...
  await gate(names[3]!,async()=>{
    const hash=await define('memory',`#[loom::def] pub fn main()->bool {let mut bytes=Vec::<u8>::new();bytes.try_reserve_exact(300*1024*1024).is_err()}`);
    const reply=await call(hash);
    assert(reply.ok&&reply.result===true,'oversized allocation not refused: '+JSON.stringify(reply));
  });
  await gate(names[4]!,async()=>{
    const hash=await define('panic',`use std::sync::atomic::{AtomicU32,Ordering}; #[loom::def] pub fn main()->u32 {let borrowed=AtomicU32::new(0);loom::scope(|s| {let spin=s.fork(||loop {borrowed.fetch_add(1,Ordering::Relaxed);std::hint::spin_loop();}).expect("fork");let fail=s.fork(||panic!("child trap control")).expect("fork");let _:()=fail.join().expect("join");let _:()=spin.join().expect("join");0}).expect("scope")}`);
  ...
  await gate(names[5]!,async()=>{
    const hash=await define('self-fork',`#[loom::def] pub fn main(depth:u32)->u32 {if depth==0 {1} else {let job=loom::fork(MAIN_DEF,depth-1).expect("fork");loom::join(vec![job]).expect("join")[0]+1}}`);
