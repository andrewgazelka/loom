...
try {
  ...
  if(process.env.LOOM_BENCH_DB&&process.env.LOOM_ARTIFACT_EVICTOR) {
    ...
    const child=Bun.spawn([process.env.LOOM_ARTIFACT_EVICTOR,process.env.LOOM_BENCH_DB,'heck',logsRef],{stdout:'pipe',stderr:'pipe'});
    ...
    assert(removed.name==='heck'&&removed.build_log_hash===logsRef&&typeof removed.dependency_graph==='string'&&/^[0-9a-f]{64}$/.test(removed.dependency_graph)&&typeof removed.key==='string'&&Number.isSafeInteger(removed.removed_outputs)&&Number(removed.removed_outputs)>0,'Evictor did not verify removal from this build graph');
