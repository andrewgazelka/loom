...
try {
  ...
  if(process.env.LOOM_BENCH_DB&&process.env.LOOM_ARTIFACT_EVICTOR) {
    const child=Bun.spawn([process.env.LOOM_ARTIFACT_EVICTOR,process.env.LOOM_BENCH_DB,'heck'],{stdout:'pipe',stderr:'pipe'});
    ...
    assert(removed.name==='heck'&&typeof removed.key==='string'&&Number.isSafeInteger(removed.removed_outputs)&&Number(removed.removed_outputs)>0,'Evictor did not verify removal');
