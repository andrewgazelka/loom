...
function definition(seq: number, hash: string, title = "worker"): JournalRow {
  return {id:`event-${seq}`,seq,kind:"defined",title,language:"rust",value:{hash},metadata:{seq}};
}
test("identical identities compact at latest occurrence while preserving provenance", () => {
  const original = [definition(1,"a"), {id:"eval",seq:2,kind:"evaluated",title:"call",language:"rust"}, definition(3,"a")];
