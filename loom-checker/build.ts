...
const adapter = join(root, 'loom-guest-ts/adapter.ts');
...
const {component} = await componentize(source, {witPath:join(root,'loom-wit/handler.wit'),worldName:'handler',disableFeatures:['stdio','random','clocks','http','fetch-event']});
