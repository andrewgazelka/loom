...
pub(crate) fn lower(file: &mut syn::File, deps: &mut BTreeMap<String, String>, diagnostics: &mut Vec<Diagnostic>) {
    struct Lower<'a> { deps: &'a mut BTreeMap<String, String>, diagnostics: &'a mut Vec<Diagnostic> }
    impl VisitMut for Lower<'_> {
        fn visit_expr_mut(&mut self, expression: &mut syn::Expr) {
            visit_mut::visit_expr_mut(self, expression);
            let syn::Expr::Call(call) = expression else { return };
            let syn::Expr::Path(function) = call.func.as_ref() else { return };
            let segments = &function.path.segments;
            if segments.len() != 2 || segments[0].ident != "loom" || segments[1].ident != "handle_with" { return }
            let hash = match call.args.first() {
                Some(syn::Expr::Lit(syn::ExprLit { lit: syn::Lit::Str(value), .. })) => value.value(),
                _ => String::new(),
            };
            if call.args.len() != 2 || hash.len() != 64 || !hash.bytes().all(|byte| byte.is_ascii_hexdigit()) {
            ...
            if self.deps.get(&alias).is_some_and(|existing| existing != &hash) {
                self.diagnostics.push(super::diagnostic(Lang::Rust, "LOOM_HANDLER_HASH", "handler dependency alias conflicts with its content hash"));
                return;
            }
...
mod tests {
    ...
    fn literal_reference_is_an_identity_edge() {
        ...
        let mut file = syn::parse_file(&format!("fn main() {{ loom::handle_with(\"{hash}\", || 7); }}")).unwrap();
        let mut deps = BTreeMap::new(); let mut errors = Vec::new();
