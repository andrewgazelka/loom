...
use loom_actor::{Behavior, Cap, ChildSpec, ChildType, Config, Ctx, DefaultEffects, Io, Node, Rights, Trap, builtin::{BehaviorInfo, Counter}};
...
struct Broadcast { targets: Vec<Cap> }
#[async_trait]
impl Behavior for Broadcast {
    fn hash(&self) -> &str { "broadcast-v1" }
    fn schema(&self) -> &str { "" }
    async fn handle(&self, cx: &mut Ctx<'_>, msg: &[u8]) -> Result<(), Trap> {
        for t in &self.targets { cx.send(t, msg).await?; }
        Ok(())
    }
}
...
impl Behavior for Noop {
    fn hash(&self) -> &str { "noop-v1" }
    fn schema(&self) -> &str { "" }
    async fn handle(&self, _cx: &mut Ctx<'_>, _msg: &[u8]) -> Result<(), Trap> { Ok(()) }
}

fn spec(hash: &str) -> ChildSpec { ChildSpec::new(hash, b"", ChildType::Worker) }
...
async fn scenario(io: Io, label: &str, n: usize, fanout: usize) -> anyhow::Result<()> {
    ...
    for _ in 0..fanout { receivers.push(node.spawn(&root, &spec("noop-v1")).await?); }
    node.run_until_idle().await?;

    // A: external inject -> one handler txn each (INSERT into entries).
    let t = Instant::now();
    for i in 0..n { node.send(&counter, &format!("k{i}"), b"x").await?; }
    let inject = t.elapsed();
    let t = Instant::now();
    let processed = node.run_until_idle().await?;
    let handle = t.elapsed();
    println!("{label} inject_only: {n} sends in {:.1} ms = {:.0} msg/s ({:.0} us/msg)", inject.as_secs_f64()*1e3, n as f64/inject.as_secs_f64(), inject.as_micros() as f64/n as f64);
    println!("{label} handle_sql_insert: processed={processed} in {:.1} ms = {:.0} msg/s ({:.0} us/msg)", handle.as_secs_f64()*1e3, n as f64/handle.as_secs_f64(), handle.as_micros() as f64/n as f64);

    // B: noop handler, the transaction floor.
    for i in 0..n { node.send(&noop, &format!("k{i}"), b"x").await?; }
    let t = Instant::now();
    let processed = node.run_until_idle().await?;
    let handle = t.elapsed();
    println!("{label} handle_noop: processed={processed} in {:.1} ms = {:.0} msg/s ({:.0} us/msg)", handle.as_secs_f64()*1e3, n as f64/handle.as_secs_f64(), handle.as_micros() as f64/n as f64);
    ...
    for i in 0..n { node.send(&fwd, &format!("h{i}"), b"x").await?; }
    let t = Instant::now();
    let processed = node.run_until_idle().await?;
    let hop = t.elapsed();
    println!("{label} hop_a_to_b: processed={processed} (expect {}) in {:.1} ms = {:.0} hops/s ({:.0} us/hop)", 2*n, hop.as_secs_f64()*1e3, n as f64/hop.as_secs_f64(), hop.as_micros() as f64/n as f64);

    let m = n / fanout.max(1);
    for i in 0..m { node.send(&bcast, &format!("b{i}"), b"x").await?; }
    let t = Instant::now();
    let processed = node.run_until_idle().await?;
    let bc = t.elapsed();
    let deliveries = m * fanout;
    println!("{label} broadcast_{fanout}: {m} msgs -> {deliveries} deliveries, processed={processed} in {:.1} ms = {:.0} deliveries/s ({:.0} us/delivery)", bc.as_secs_f64()*1e3, deliveries as f64/bc.as_secs_f64(), bc.as_micros() as f64/deliveries as f64);
    node.close().await?;
    Ok(())
}
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("host={} n={n} fanout={fanout}", std::process::Command::new("hostname").output().map(|o| String::from_utf8_lossy(&o.stdout).trim().to_owned()).unwrap_or_default());
