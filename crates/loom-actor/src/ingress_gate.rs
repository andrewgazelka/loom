...
impl Node {
    ...
    pub fn pause_shipping(&self) {
        self.shipping.paused.store(true, Ordering::Release);
    }
    pub fn resume_shipping(&self) {
        self.shipping.paused.store(false, Ordering::Release);
    }
    ...
    pub(crate) async fn await_receiver_ship(&self, id: &str) -> Result<()> {
        ...
        loop {
            ...
            if self.shipping.get(id)?.changes.is_some_and(|published| published >= required) {
                return Ok(());
            }
