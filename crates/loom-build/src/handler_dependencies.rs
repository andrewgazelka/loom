...
pub(crate) fn validate(
    ...
    for (alias, hash) in &definition.deps {
        if !alias.starts_with("loom_handler_") {
            continue;
        }
        ...
        let target = dependencies.get(hash).ok_or_else(|| {
            BuildError::Rejected(format!("handler definition {hash} is not stored"))
        })?;
        if target.lang != loom_proto::Lang::Rust {
            return Err(BuildError::Rejected(format!(
                "handler {hash} must be a Rust definition sharing the caller's SDK"
            )));
        }
        let source = if target.source.trim_start().starts_with('{') {
            let bundle: SourceBundle = serde_json::from_str(&target.source)
                .map_err(|error| BuildError::Rejected(error.to_string()))?;
            bundle
                .files
                .get("src/lib.rs")
                .and_then(SourceFile::as_text)
                .ok_or_else(|| BuildError::Rejected(format!("handler {hash} requires src/lib.rs")))?
                .to_owned()
        } else {
            target.source.clone()
        };
...
mod tests {
    ...
    fn pinned_handler_requires_stored_public_export() {
        ...
        caller
            .deps
            .insert(format!("loom_handler_{hash}"), hash.clone());
        assert!(
            validate(&caller, &BTreeMap::new())
                .unwrap_err()
                .to_string()
                .contains("not stored")
        );
        ...
        assert!(
            validate(&caller, &dependencies)
                .unwrap_err()
                .to_string()
                .contains("pub fn handle")
        );
    ...
    fn upgrading_a_pin_preserves_its_source_alias() {
        ...
        caller
            .deps
            .insert(format!("loom_handler_{old_hash}"), new_hash.clone());
        let target = definition(
            &new_hash,
            "pub fn handle(op: loom::Effect, k: loom::Continuation) -> loom::Reply { loom::Reply::Forward }",
        );
