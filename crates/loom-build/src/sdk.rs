...
fn seed_sdk(original: &Lock, current: &Lock) -> Result<Vec<u8>, BuildError> {
    let sdk = current.reachable(&sdk_roots(current), false);
    let mut raw = original.raw.clone();
    let array = raw
        .get_mut("package")
        .and_then(toml::Value::as_array_mut)
        .ok_or_else(|| BuildError::Rejected("missing package array".into()))?;
    for package in &current.packages {
        if !sdk.contains(&package.key) {
            continue;
        }
        if let Some(index) = original
            .packages
            .iter()
            .position(|old| old.key == package.key)
        {
            if sdk_package(&package.key) {
                array[index] = package.raw.clone();
            }
        } else {
            array.push(package.raw.clone());
        }
    }
...
pub(crate) async fn reconcile(job: Rebuild<'_>) -> Result<(), BuildError> {
    ...
    let manifest: toml::Value =
        toml::from_str(&fs::read_to_string(job.directory.join("Cargo.toml")).await?)
            .map_err(|error| BuildError::Rejected(error.to_string()))?;
    let seeded = seed_sdk(&original, &current)?;
    if Lock::parse(&seeded)?.raw == original.raw {
        return Ok(());
    }
    ...
    fs::write(&lock_path, seeded).await?;
