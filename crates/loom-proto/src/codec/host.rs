...
use serde::{
    Serialize,
    de::{self, DeserializeOwned, DeserializeSeed, IntoDeserializer, Visitor},
};
...
pub fn encode_host_array<'a>(
    values: impl IntoIterator<Item = &'a [u8], IntoIter: ExactSizeIterator>,
    ...
    let mut bytes =
        serde_ipld_dagcbor::to_vec(&(values.len() as u64)).map_err(|error| error.to_string())?;
    bytes[0] |= 0x80;
    for value in values {
...
pub fn decode_host<T: DeserializeOwned>(bytes: &[u8]) -> Result<T, String> {
    ...
    let result = T::deserialize(HostDeserializer {
        inner: &mut decoder,
        depth: 0,
    })
...
impl<'de, V: Visitor<'de>> Visitor<'de> for EnumVisitor<V> {
    ...
    fn visit_str<E: de::Error>(self, value: &str) -> Result<Self::Value, E> {
        self.inner.visit_enum(value.into_deserializer())
    }
    ...
    fn visit_map<A: de::MapAccess<'de>>(self, map: A) -> Result<Self::Value, A::Error> {
        self.inner
            .visit_enum(de::value::MapAccessDeserializer::new(map))
    }
}
struct HostVisitor<V> {
    inner: V,
    depth: usize,
}
macro_rules! visit_integer {
    ($method:ident, $ty:ty) => {
        fn $method<E: de::Error>(self, value: $ty) -> Result<Self::Value, E> {
            if !(-super::MAX_SAFE_INTEGER..=super::MAX_SAFE_INTEGER).contains(&(value as i128)) {
...
impl<'de, V: Visitor<'de>> Visitor<'de> for HostVisitor<V> {
    ...
    fn visit_u128<E: de::Error>(self, value: u128) -> Result<Self::Value, E> {
        if value > super::MAX_SAFE_INTEGER as u128 {
            return Err(E::custom("integer exceeds JavaScript safe range"));
        }
    ...
    fn visit_f32<E: de::Error>(self, value: f32) -> Result<Self::Value, E> {
        if !value.is_finite() || (value == 0.0 && value.is_sign_negative()) {
            return Err(E::custom("invalid Value float"));
        }
    ...
    fn visit_some<D: de::Deserializer<'de>>(self, inner: D) -> Result<Self::Value, D::Error> {
        self.inner.visit_some(HostDeserializer {
            inner,
            depth: self.depth,
        })
    }
    fn visit_seq<A: de::SeqAccess<'de>>(self, inner: A) -> Result<Self::Value, A::Error> {
        self.inner.visit_seq(HostSequence {
            inner,
            depth: self.depth + 1,
        })
    }
    fn visit_map<A: de::MapAccess<'de>>(self, inner: A) -> Result<Self::Value, A::Error> {
        self.inner.visit_map(HostMap {
            inner,
            depth: self.depth + 1,
        })
    }
    fn visit_newtype_struct<D: de::Deserializer<'de>>(
        ...
        impl<'de> Visitor<'de> for CidVisitor {
            ...
            fn visit_bytes<E: de::Error>(self, bytes: &[u8]) -> Result<Cid, E> {
                Cid::try_from(bytes).map_err(E::custom)
            }
        }
        let cid = decoder.deserialize_bytes(CidVisitor)?;
        self.inner
            .visit_map(ReferenceMap {
                reference: Some(cid.to_string()),
                key: true,
            })
...
impl<'de, S: DeserializeSeed<'de>> DeserializeSeed<'de> for HostSeed<S> {
    ...
    fn deserialize<D: de::Deserializer<'de>>(self, inner: D) -> Result<Self::Value, D::Error> {
        self.inner.deserialize(HostDeserializer {
            inner,
            depth: self.depth,
        })
    }
...
impl<'de, A: de::SeqAccess<'de>> de::SeqAccess<'de> for HostSequence<A> {
    ...
    fn next_element_seed<S: DeserializeSeed<'de>>(
        ...
        self.inner.next_element_seed(HostSeed {
            inner: seed,
            depth: self.depth,
        })
    }
...
impl<'de, A: de::MapAccess<'de>> de::MapAccess<'de> for HostMap<A> {
    ...
    fn next_key_seed<S: DeserializeSeed<'de>>(
        ...
        self.inner.next_key_seed(HostKeySeed { inner: seed })
    }
    fn next_value_seed<S: DeserializeSeed<'de>>(&mut self, seed: S) -> Result<S::Value, A::Error> {
        self.inner.next_value_seed(HostSeed {
            inner: seed,
            depth: self.depth,
        })
    }
...
struct HostKeySeed<S> {
    inner: S,
}
impl<'de, S: DeserializeSeed<'de>> DeserializeSeed<'de> for HostKeySeed<S> {
    type Value = S::Value;
    fn deserialize<D: de::Deserializer<'de>>(self, decoder: D) -> Result<Self::Value, D::Error> {
        struct KeyVisitor<S> {
            inner: S,
        }
        impl<'de, S: DeserializeSeed<'de>> Visitor<'de> for KeyVisitor<S> {
            type Value = S::Value;
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                f.write_str("a Value map key")
            }
            fn visit_str<E: de::Error>(self, value: &str) -> Result<Self::Value, E> {
                if value == "$ref" {
                    return Err(E::custom("wire references must use DAG-CBOR tag 42"));
                }
                self.inner.deserialize(value.into_deserializer())
            }
            fn visit_borrowed_str<E: de::Error>(self, value: &'de str) -> Result<Self::Value, E> {
                if value == "$ref" {
                    return Err(E::custom("wire references must use DAG-CBOR tag 42"));
        ...
        decoder.deserialize_str(KeyVisitor { inner: self.inner })
    }
}
struct ReferenceMap {
...
impl<'de> de::MapAccess<'de> for ReferenceMap {
    ...
    fn next_value_seed<S: DeserializeSeed<'de>>(
        ...
        let reference = self
            .reference
            .take()
            .ok_or_else(|| de::Error::custom("CID value already consumed"))?;
        seed.deserialize(reference.into_deserializer())
    }
    fn size_hint(&self) -> Option<usize> {
        Some(usize::from(self.key))
    }
}
