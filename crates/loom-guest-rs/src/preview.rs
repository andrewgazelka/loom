...
use crate::{EffectError, Reply, Value, fs};
...
fn normalized_path(path: &str) -> Result<String, EffectError> {
    ...
    for part in path.split('/') {
        if part == ".." {
            return Err("parent traversal is forbidden".into());
        }
...
fn dispatch(
    ...
    let existing = pending
        .iter()
        .position(|item| item.change.machine == args.machine && item.change.path == args.path);
    match label {
        "fs.write" => {
            let content = args
                .content
                .ok_or_else(|| "fs.write requires string content".to_owned())?;
            if content.len() > 64 * 1024 * 1024 {
                return Err("file exceeds 64 MiB write limit".into());
            }
            ...
            if let Some(index) = existing {
                ...
                let before = fs::read_optional(&args.machine, &args.path)?
                    .as_deref()
                    .map(capture)
                    .transpose()?;
                pending.push(Pending {
                    change: FilesystemChange {
                        ...
                        after: Some(after),
                        captured: true,
                        content_encoding: "dag-cbor-string".into(),
                    },
                    content,
                });
            }
        ...
        "fs.read" | "fs.read_optional" => {
            if let Some(index) = existing {
                return Ok(Value::String(pending[index].content.clone()));
            }
            crate::perform(
                label,
                serde_json::json!({"machine":args.machine,"path":args.path}),
            )
        }
    ...
    F: FnOnce() -> R,
{
    ...
    let result = crate::handle(
        ["fs.write", "fs.read", "fs.read_optional"],
        |op, _continuation| {
            let args = op
                .arg::<FileArgs>()
                .expect("invalid preview filesystem arguments");
            Reply::Resume(
                dispatch(&op.name, args, &mut pending).expect("filesystem preview failed"),
            )
        },
        body,
    )?;
    Ok(Preview {
        result,
        filesystem_changes: pending
            .into_iter()
            .map(|item| item.change)
            .filter(|change| change.before != change.after)
            .collect(),
        filesystem_capture: FilesystemCapture {
            scope: "preview".into(),
            preview: true,
        },
    })
