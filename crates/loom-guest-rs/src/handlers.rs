...
impl Continuation {
    pub fn resume<T: Serialize>(self, value: T) -> Result<(), EffectError> {
        ...
        status(
            host_resume(self.state.id, &bytes),
            "continuation resume refused",
        )
    }
...
impl Drop for Continuation {
    fn drop(&mut self) {
        ...
        if previous & (CALLBACK | DISARMED | DROPPED) == 0 && host_drop(self.state.id) != 0 {
            abort_execution();
        }
    }
}
fn finish_callback(state: &ContinuationState, reply: &Reply) {
    match reply {
        ...
        Reply::Deferred => {
            ...
            if previous & DROPPED != 0 && previous & DISARMED == 0 && host_drop(state.id) != 0 {
                abort_execution();
            }
        }
    }
...
struct Handler<H> {
    callback: UnsafeCell<H>,
    entered: AtomicBool,
}
struct Installed<H> {
    frame: u64,
    _handler: Box<Handler<H>>,
}
...
pub fn handle<'a, H, F, R>(
    labels: impl AsRef<[&'a str]>,
    handler: H,
    body: F,
...
unsafe fn run<H: FnMut(Effect, Continuation) -> Reply>(
    data: *mut (),
    id: u64,
    op: Effect,
) -> Reply {
    ...
    if handler.entered.swap(true, Ordering::Acquire) {
        abort_execution();
    }
    let state = Arc::new(ContinuationState {
        id,
        state: AtomicU8::new(CALLBACK),
    });
...
mod tests {
    ...
    fn deferred_retained_continuation_is_not_prematurely_dropped() {
        ...
        drop(continuation);
        assert_eq!(
            DROPS.lock().unwrap().iter().filter(|id| **id == 2).count(),
            1
        );
    }
    #[test]
    ...
    fn retained_continuation_is_disarmed_by_inline_reply() {
        ...
        assert_eq!(
            continuation.resume(7).unwrap_err(),
