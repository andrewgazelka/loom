...
use crate::{EffectError, scoped::{join_task, start_task}};
use std::{cell::UnsafeCell, sync::{Arc, atomic::{AtomicBool, Ordering}}};
...
impl<F, T> TaskResult<T> for Task<F, T> {
    fn slot(&self) -> &ResultSlot<T> { &self.slot }
}
    ...
    T: Send + 'static,
{
    let task = Arc::new(Task {
        ...
        slot: ResultSlot { result: UnsafeCell::new(None), published: AtomicBool::new(false) },
    });
...
impl<T> JoinHandle<T> {
    ...
    pub fn join(self) -> Result<T, EffectError> {
        if join_task(self.id) != 0 { return Err(task_error(self.id)); }
...
fn task_error(id: u64) -> EffectError { format!("shared job {id}: task failed") }
...
mod tests {
    ...
    fn join_returns_value_and_handle_is_send() {
        ...
        assert_eq!(spawn(|| String::from("done")).unwrap().join().unwrap(), "done");
    }
    ...
    fn handle_moves_into_scoped_child() {
        ...
        crate::scope(|scope| {
            assert_eq!(scope.spawn(move || handle.join().unwrap()).unwrap().join().unwrap(), 42);
        });
    }

    #[test]
    fn dropped_handle_leaves_task_storage_alive() {
        struct Witness { dropped: std::sync::mpsc::Sender<()> }
        impl Drop for Witness {
            fn drop(&mut self) { self.dropped.send(()).unwrap(); }
        }
        let (release, wait) = std::sync::mpsc::channel();
        let (dropped, observed) = std::sync::mpsc::channel();
        let witness = Witness { dropped };
        let handle = spawn(move || { wait.recv().unwrap(); witness }).unwrap();
        let id = handle.id;
        drop(handle);
