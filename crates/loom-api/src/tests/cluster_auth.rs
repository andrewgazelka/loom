...
async fn ingress_credentials_are_route_exclusive_before_handler_execution() {
    ...
    let app = protect(
        ...
        Authorizer::single("user-token".into())
            .unwrap()
            .with_ingress_bearer(Some("cluster-token".into())),
    );
    struct Case {
        path: &'static str,
        token: &'static str,
    ...
    for case in [
        ...
        assert_eq!(
            ...
            0,
            "rejected ingress reached the write handler"
        );
    }
    let response = app
        .oneshot(
    ...
    assert_eq!(
        writes.load(Ordering::SeqCst),
        1,
        "valid ingress control never reached the handler"
    );
}
