...
fn uncertain_send_is_not_reported_as_a_dead_letter() {
    ...
    let service = Service::new(
        Store::memory().unwrap(), PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../.."), vec![Lang::Rust],
    ).unwrap();
    let pending = service.response(Err(ActorMessageFailure {
        id: "actor".into(), seq: 7, cause: "delivery still pending".into(), outcome: MessageOutcome::Pending,
    }.into()));
    assert!(!pending.ok);
    ...
    let failed = service.response(Err(ActorMessageFailure {
        id: "actor".into(), seq: 7, cause: "trap".into(), outcome: MessageOutcome::Failed,
    }.into()));
...
async fn read_scope_cannot_create_view_directly_or_through_command() {
    let authorizer = Authorizer::new(vec![crate::auth::TokenConfig {
        token: "reader".into(), scopes: [Scope::Read].into_iter().collect(),
    }]).unwrap();
    let service = Service::new(
        Store::memory().unwrap(), PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../.."), vec![Lang::Rust],
    ).unwrap().scoped(authorizer.authenticate("reader").unwrap());
    let args = json!({"actor":"a","table":"t","template":"h","order_by":[]});
    for request in [
        CommandRequest { session: None, command: "view".into(), args: args.clone() },
        CommandRequest { session: None, command: "command".into(), args: json!({"command":"view","args":args}) },
    ] {
        let response = service.command(request).await;
        assert!(!response.ok);
        assert_eq!(response.result["code"], "forbidden");
        assert!(response.result["error"].as_str().unwrap().contains("Execute"));
    }
}
#[tokio::test]
