...
impl ActorService {
    async fn forward_command(&self, verb: &str, args: &Value) -> anyhow::Result<Option<Value>> {
        ...
        let op = DeliveryOp::Command { target: target.into(), verb: verb.into(), args: args.clone() };
        let acks = self.node.forward(&addr, &[op]).await.map_err(error)?;
        let ack = acks.into_iter().next().ok_or_else(|| error(format!("actor {target} seq -1: missing command ack")))?;
        anyhow::ensure!(ack.ok, "actor {target} seq -1: {}", ack.error.as_deref().unwrap_or("owner refused command"));
        Ok(Some(ack.result.ok_or_else(|| error(format!("actor {target} seq -1: missing command result")))?))
    }
        ...
        let result = async {
            match self.node.resolve(&target).await? {
                Placement::Remote { node_id, addr } => return Ok(Ack {
                    ok: false, conflict: true, owner: Some(node_id), addr: Some(addr), error: None, result: None,
                }),
                Placement::Local | Placement::Unowned => {}
            }
            anyhow::ensure!(args["id"].as_str().or_else(|| args["root"].as_str()) == Some(target.as_str()),
                "actor {target} seq -1: ingress command target mismatch");
            anyhow::ensure!(matches!(verb.as_str(),
                "info" | "sql" | "lineage" | "dead_letters" | "tree" | "send" | "stop" | "restart" | "promote" | "fork" | "validate"),
            ...
            if matches!(verb.as_str(), "send" | "stop" | "restart" | "promote" | "fork" | "validate") {
                // This ack leaves the request only after the mutation is published.
                self.node.ship(&target).await?;
            }
            Ok::<_, anyhow::Error>(Ack { ok: true, conflict: false, owner: None, addr: None, error: None, result: Some(value) })
        }.await;
        result.unwrap_or_else(|failure| Ack {
            ok: false, conflict: false, owner: None, addr: None, error: Some(format!("actor {target} seq -1: {failure:#}")), result: None,
        })
    }
