...
CREATE TABLE IF NOT EXISTS source_revisions(def_hash TEXT NOT NULL REFERENCES defs(hash),source_hash TEXT NOT NULL REFERENCES cas(hash),since_seq INTEGER NOT NULL REFERENCES definition_records(seq),PRIMARY KEY(def_hash,source_hash));
