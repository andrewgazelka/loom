...
unsafe fn counter(memory: &SharedMemory, address: u32) -> Result<&AtomicU32> {
    ...
    anyhow::ensure!(start.checked_add(4).is_some_and(|end| end <= memory.data_size()), "borrow witness outside shared memory");
    let pointer = memory.data()[start].get().cast::<u32>();
    anyhow::ensure!((pointer as usize) % std::mem::align_of::<AtomicU32>() == 0, "unaligned borrow witness");
...
impl Runtime {
    ...
    pub async unsafe fn verify_borrowed_handler_cancellation(&self, bytes: &[u8]) -> Result<Value> {
        anyhow::ensure!(loom_proto::component_protocol::is_core_current(bytes), "cancellation control requires current core ABI");
        ...
        for import in module.imports() {
            if let ExternType::Memory(ty) = import.ty() {
                anyhow::ensure!(import.module() == "env" && import.name() == "memory" && memory_type.is_none(), "control requires exactly env.memory");
                anyhow::ensure!(ty.is_shared() && !ty.is_64() && ty.maximum().is_some_and(|pages| pages * 65536 <= MAX_MEMORY), "invalid control memory limits");
                memory_type = Some(ty);
            }
        }
        let memory = SharedMemory::new(&self.inner.core_engine, memory_type.context("missing shared memory")?).map_err(error)?;
        ...
        let execution = Arc::new(Execution {
            ...
            runtime: self.clone(), module, memory, effects: EffectContext { trace: Some(trace.clone()), ..Default::default() }, pure: false,
            ...
            cancelled: AtomicBool::new(false), cancellation: Notify::new(), deadline: Instant::now() + Duration::from_secs(10),
        });
        let mut cleanup = Cleanup { execution: Some(execution.clone()) };
        let child = execution.clone();
        let task_scope = scope.clone();
        let (sender, mut receiver) = tokio::sync::oneshot::channel();
        execution.schedule(async move {
            let result = child.invoke(task_scope, Invocation::Call { args: encode(&json!([])).unwrap() }).await;
            let _ = sender.send(result);
        })?;
        let address = tokio::time::timeout(Duration::from_secs(3), async {
            loop {
                for blob in trace.snapshot(None, false)?.blobs {
                    if blob.kind != loom_proto::TraceBlobKind::Descriptor { continue; }
                    let descriptor: Value = loom_proto::decode(&blob.bytes).map_err(anyhow::Error::msg)?;
                    if descriptor.get("op").and_then(Value::as_str) == Some("now") {
                        if let Some(address) = descriptor.get("args").and_then(|args| args.get("borrow_address")).and_then(Value::as_u64) {
        ...
        }).await.context("borrowed callback did not report its address")??;
        ...
        tokio::time::timeout(Duration::from_secs(3), async {
            ...
            while borrowed.load(Ordering::SeqCst) == first { tokio::task::yield_now().await; }
        }).await.context("callback was not actively updating borrowed parent storage")?;
        anyhow::ensure!(execution.handler_instance_reuses.load(Ordering::Relaxed) >= 1, "active callback did not reuse the warmed handler cache");
        anyhow::ensure!(!execution.continuations.lock().unwrap().is_empty(), "no active suspended performer");
        execution.cancel();
        tokio::time::timeout(Duration::from_secs(3), execution.drain()).await.context("actual guest Store drain timed out")?;
        cleanup.execution.take();
        anyhow::ensure!(execution.tasks.lock().unwrap().is_empty() && execution.handler_instances.lock().unwrap().is_empty(), "guest tasks or cached Stores survived drain");
        anyhow::ensure!(Arc::strong_count(&execution) == 1, "a guest Store still owns execution after drain");
        match receiver.try_recv() {
            ...
            Err(tokio::sync::oneshot::error::TryRecvError::Empty) => bail!("root completion channel is still pending after drain"),
        }
        ...
        anyhow::ensure!(borrowed.load(Ordering::SeqCst) == sentinel, "callback wrote parent storage after drain");
        Ok(json!({"scope": scope, "module_hash": module_hash, "pass": true,
