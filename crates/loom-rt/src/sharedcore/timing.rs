    ...
    pub async fn benchmark_handler_module(&self, bytes: &[u8]) -> Result<Value> {
        ...
        for import in module.imports() {
            if let ExternType::Memory(ty) = import.ty() {
                anyhow::ensure!(import.module() == "env" && import.name() == "memory" && memory_type.is_none(), "benchmark core must import exactly env.memory");
                anyhow::ensure!(ty.is_shared() && !ty.is_64() && ty.maximum().is_some_and(|pages| pages * 65536 <= MAX_MEMORY), "invalid benchmark shared memory limits");
                memory_type = Some(ty);
            }
        }
        let memory = SharedMemory::new(&self.inner.core_engine, memory_type.context("benchmark core has no shared memory")?).map_err(error)?;
        let execution = Arc::new(Execution {
            handler_instances: Mutex::new(Vec::new()), handler_instance_reuses: AtomicU64::new(0),
            handler_round_trip_us: Mutex::new(Vec::new()),
            handlers_next: AtomicU64::new(1), continuations: Mutex::new(HashMap::new()), handler_failure: Mutex::new(None),
            runtime: self.clone(), module, memory, effects: EffectContext::default(), pure: true,
        ...
        execution.schedule(async move {
            let result: Result<Vec<f64>> = async {
                ...
                let call = running.instance.get_typed_func::<(i32, i32), i64>(&mut running.store, "loom_call").map_err(error)?;
                ...
                for index in 0..10_100 {
                    if index == 100 { child.handler_round_trip_us.lock().unwrap().clear(); }
                    let start = Instant::now();
                    let packed = call.call_async(&mut running.store, (input.pointer, input.length)).await.map_err(error)? as u64;
                    ...
                    anyhow::ensure!(response == json!({"ok": 1}), "benchmark fixture returned {response}, expected 1");
                    anyhow::ensure!(running.store.data().handlers.is_empty(), "benchmark fixture leaked handler frames");
                    anyhow::ensure!(child.continuations.lock().unwrap().is_empty(), "benchmark fixture leaked continuations");
        ...
        let result = receiver.await.context("benchmark guest task ended without result");
        execution.cancel();
        execution.drain().await;
        cleanup.execution.take();
        let mut samples = result??;
        samples.sort_by(f64::total_cmp);
        Ok(json!({"scope": scope, "samples": samples.len(), "warmup_samples": 100,
...
mod tests {
    ...
    async fn actual_sdk_full_handler_round_trip() -> Result<()> {
        ...
        anyhow::ensure!(result["pass"] == true, "complete round-trip median is not under 20us");
        Ok(())
    }
}
