            ...
            |mut caller: Caller<'_, Guest>,
             ...
             -> std::result::Result<i64, wasmtime::Error> {
                let result: Result<i64> = (|| {
                    ...
                    execution.check()?;
                    anyhow::ensure!(
                        caller.data().handlers.len() < MAX_JOBS,
                        "handler stack limit exceeded"
                    );
                    ...
                    let labels = if value.is_null() {
                        None
                    } else {
                        Some(
                            value
                                .as_array()
                                .context("handler labels must be an array or null")?
                                .iter()
                                .map(|label| {
                                    label
                                        .as_str()
                                        .map(str::to_owned)
                                        .context("handler label must be a string")
                                })
                    ...
                    anyhow::ensure!(
                        ...
                        "handler identity exhausted"
                    );
                    let frame = Arc::new(HandlerFrame {
                        id,
                        owner: caller.data().scope.clone(),
                        function,
                        data,
                        labels,
             ...
             -> std::result::Result<i32, wasmtime::Error> {
                let result: Result<i32> = (|| {
                    ...
                    let state = execution
                        ...
                        .get(&(id as u64))
                        .cloned()
                        .context("unknown or consumed continuation")?;
                    let bytes = copy_out(&execution.memory, pointer as u32, length as u32)?;
                    let value: Value = loom_proto::decode(&bytes).map_err(anyhow::Error::msg)?;
...
pub(super) fn dispatch<'a>(
    caller: &'a mut Caller<'_, Guest>,
    descriptor: &'a Value,
    occurrence: i64,
) -> futures::future::BoxFuture<'a, Result<Option<Vec<u8>>>> {
    Box::pin(async move {
        let execution = caller.data().execution.clone();
        ...
        for index in (0..frames.len()).rev() {
            ...
            caller.data_mut().permit = Some(execution.permit().await?);
            let id = execution.handlers_next.fetch_add(1, Ordering::Relaxed);
            let state = Arc::new(ContinuationState {
            ...
            let callback_scope = format!(
                "{}/effect:{occurrence}/handler:{index}",
                caller.data().scope
            );
            let result = invoke(
                caller,
            ...
            let result: Result<Option<std::result::Result<Value, String>>> = async {
                let reply = result?;
                anyhow::ensure!(
                    reply.as_object().is_some_and(|object| object.len() == 1),
                ...
                if let Some(value) = reply.get("resume") {
                    ...
                    anyhow::ensure!(
                        frame.labels.is_none(),
                        "selected handler label cannot forward"
                    );
                    anyhow::ensure!(
                        state.result.lock().unwrap().is_none(),
                        "consumed continuation cannot forward"
                    );
                    return Ok(None);
                } else {
                    anyhow::ensure!(
                        reply.get("deferred") == Some(&Value::Null),
                        "invalid handler reply"
                    );
                }
            ...
            match result {
                ...
                Ok(Some(Err(message))) if message == "continuation dropped" => {
                    ...
                    caller.data_mut().last_effect_error =
                        Some(format!("handler frame {}: {message}", frame.id));
...
impl HandlerInstance {
    async fn reset(&mut self, scope: String, outer: &[Arc<HandlerFrame>]) -> Result<()> {
        ...
        instance
            .get_global(&mut self.running.store, "__loom_stack_low")
            .context("missing stack lower bound")?
            .set(
                &mut self.running.store,
                wasmtime::Val::I32(self.stack as i32),
            )
            .map_err(error)?;
        instance
            .get_global(&mut self.running.store, "__loom_stack_high")
            .context("missing stack upper bound")?
            .set(
                &mut self.running.store,
                wasmtime::Val::I32((self.stack + STACK_BYTES) as i32),
            )
            .map_err(error)?;
        instance
            .get_global(&mut self.running.store, "__stack_pointer")
            .context("missing stack pointer")?
            .set(
                &mut self.running.store,
                wasmtime::Val::I32((self.stack + STACK_BYTES) as i32),
            )
            .map_err(error)?;
        instance
            .get_typed_func::<i32, ()>(&mut self.running.store, "__wasm_init_tls")
            .map_err(error)?
            .call_async(&mut self.running.store, self.tls as i32)
            .await
            .map_err(error)?;
        Ok(())
    }
...
async fn invoke(
    ...
    let op = encode(
        &json!({"name": descriptor.get("op"), "args": descriptor.get("args").unwrap_or(&Value::Null)}),
    )?;
    let input = running.input_encoded(&op).await?;
    let function = running
        .instance
        .get_typed_func::<(i32, i32, i64, i32, i32), i64>(&mut running.store, "loom_handler_run")
        .map_err(error)?;
    let packed = function
        .call_async(
            &mut running.store,
    ...
    if let Some(unused) = unused {
        ...
        caller.data_mut().permit = Some(execution.permit().await?);
        let free = caller
            .get_export("loom_dealloc")
            .and_then(|export| export.into_func())
            .context("missing loom_dealloc")?
            .typed::<(i32, i32, i32), ()>(&*caller)
            .map_err(error)?;
        free.call_async(&mut *caller, (stack as i32, STACK_BYTES as i32, 16))
            .await
            .map_err(error)?;
        if tls_size != 0 {
            free.call_async(
                &mut *caller,
                (tls as i32, tls_size as i32, tls_align as i32),
            )
            .await
            .map_err(error)?;
        }
    ...
    async fn cached_instance_resets_tls_stack_context_and_releases_execution_cycle() -> Result<()> {
        ...
        let module = Module::new(
            ...
            (func (export "__wasm_init_tls") (param i32)
                local.get 0 i32.const 7 i32.atomic.store8))"#,
        )
        .map_err(error)?;
        let memory = SharedMemory::new(
            &runtime.inner.core_engine,
            wasmtime::MemoryType::shared(8, 8),
        )
        .map_err(error)?;
        ...
        let running = execution
            .instantiate(
                "old".into(),
                Some(Allocation {
                    stack: 4096 + STACK_BYTES,
                    tls: 400_000,
                }),
                vec![frame.clone()],
            )
            .await?;
        let mut cached = HandlerInstance {
            running,
            stack: 4096,
            tls: 400_000,
            tls_size: 16,
        ...
        cached
            .running
            .instance
            .get_global(&mut cached.running.store, "__stack_pointer")
            .unwrap()
            .set(&mut cached.running.store, wasmtime::Val::I32(5000))
            .map_err(error)?;
        cached.park();
        assert_eq!(
            Arc::strong_count(&frame),
            1,
            "parked Store retained a borrowed frame"
        );
        assert_eq!(execution.permits.available_permits(), 8);
        cached.reset("new".into(), &[]).await?;
        assert_eq!(
            copy_out(&execution.memory, cached.tls, 1)?,
            vec![7],
            "TLS initializer did not run on reuse"
        );
        ...
        assert_eq!(
            cached
                ...
                .get_global(&mut cached.running.store, "__stack_pointer")
                .unwrap()
                .get(&mut cached.running.store)
                .i32(),
            Some((4096 + STACK_BYTES) as i32)
        );
        cached.park();
        ...
        assert_eq!(
            ...
            1,
            "cached Store leaked the execution"
        );
        Ok(())
    }
